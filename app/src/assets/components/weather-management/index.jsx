...
export default function WeatherManagement({
  ...
  // ✅ เอา greenhouseId จาก query.r มาใช้ทั้งเรียก API และตั้ง DOM id
  const greenhouseId = useMemo(() => query?.r ?? "default", [query?.r]);
  const Query = useMemo(() => ({ r: greenhouseId }), [greenhouseId]);
  ...
  // ===== Fallback: ให้มี “โครงกราฟ” แม้ไม่มีข้อมูล =====
  const currentField = columns[selectedTab]?.field;
  const hasDataKey =
    chartDatas.length > 0 && chartDatas.some((d) => d[currentField] !== undefined && d[currentField] !== null);
    ...
    <Stack width={"100%"} height={"100%"} alignItems={"center"}>
      <Grid container width={"100%"} height={"100%"} justifyContent={"center"}>
        <Grid size={{ sm: 12, md: 6 }} height={isMediaSm ? "55%" : "100%"} minHeight={isMediaSm ? "300px" : 0}>
          ...
          {/* ⬇️ DOM id ผูกกับ greenhouseId เพื่อให้ Export เลือกถูกโรงเรือน */}
          <Stack
            id={`weather-chart-export-${greenhouseId}`}
            width={"100%"}
        ...
        <Grid size={{ sm: 12, md: 6 }} height={isMediaSm ? "45%" : "100%"} width={"100%"}>
          {/* ⬇️ DOM id ผูกกับ greenhouseId เพื่อให้ Export เลือกถูกโรงเรือน */}
          <Stack id={`weather-table-export-${greenhouseId}`} width={"100%"} height={"100%"} alignItems={"center"}>
