...
const DetailEdit = ({ Ref, setRef, setDetailData, type, id_form, id_from_plant, session }) => {
    ...
    const UrlFecth = "/api/admin/form/edit/get"
    const DataFetch = {
        id_form: id_form,
        type_form: type
    }
    ...
    const ApiResultCheck = async (id_edit, resultBt) => {
        if (CheckEmplyBecause()) {
            setOpenreport(1)
            const result = await clientMo.put("/api/admin/form/edit/change/status", {
                status: resultBt,
                                                    ...
                                                    >
                                                        {key === 0 ? (
                                                            ...
                                                            <>
                                                                ...
                                                                {val.id_doctor_edit ? `หมอพืช` : val.id_admin ? "ผู้ดูแลระบบ" : "เกษตรกร"}
                                                            </>
                                                        )}
                                                    </a>
                                                ))
