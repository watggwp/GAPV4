...
// const ManageAdminPage = ({RefOnPage , id_table , type , status , setBecause , TabOn , session , ReloadFetch}) => {
...
//             setOpen(1)
//             const result = await clientMo.post("/api/admin/manage/admin" , data)
//             if(result === "133") {
    ...
    const Submit = async () => {
        if (BecauseRef.current.value && PasswordRef.current.value) {
            ...
            if (result === "133") {
                ...
                setText("บัญชีนี้ถูกลบไปแล้ว");
                setStatus(2);
            } else if (result === "cannot delete last admin") {
                setText("ไม่สามารถลบผู้ดูแลระบบบัญชีนี้ได้");
                setStatus(3);
            } else if (result === "because") {
                setText("เกิดปัญหาทางเซิร์ฟเวอร์");
                setStatus(3);
