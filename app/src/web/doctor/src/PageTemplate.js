...
const PageTemplate = ({socket , addHref = false , HrefData , modify , auth , session , TabOn}) => {
    const [StatusPage , setStatus] = useState({
        status :    HrefData.get() === "list?default" ? "default" :
                    ...
                    HrefData.get() === "listdoctor?doctor=pop" ? "doctor" :
                    HrefData.get() === "listdoctor?deletedoctor=pop" ? "deletedoctor" :
    ...
    <InsertStatisticsProvider>
        <div className="menu-page">
            {/* แสดงปุ่มเพิ่มข้อมูล */}
            {["default", "doctor", "plant", "station", "group", "chemical", "pest", "report"].includes(StateOnPage.status) && (
...
{StateOnPage.status !== "group" && (
    <select
        ...
        className={`dropdown-status ${
            ["default", "delete", "doctor", "deletedoctor"].includes(StateOnPage.status) ? "dropdown-status-default" :
            ["plant", "chemical", "pest", "station"].includes(StateOnPage.status) ? "dropdown-status-plant" :
            ["listlocation", "graph", "statistics"].includes(StateOnPage.status) ? "dropdown-status-location" : ""
        }`}
        onChange={(e) => ChangeStatus(e.target.value)}
        value={StateOnPage.status}
    >
        {["default", "delete", "doctor", "deletedoctor"].includes(StateOnPage.status) && (
            <>
                <option value="default">แสดงบัญชีผู้ส่งเสริมที่ยังไม่ถูกลบ</option>
                <option value="delete">แสดงบัญชีผู้ส่งเสริมที่ถูกลบ</option>
                <option value="doctor">แสดงบัญชีผู้ดูแลระบบที่ยังไม่ถูกลบ</option>
                <option value="deletedoctor">แสดงบัญชีผู้ดูแลระบบที่ถูกลบ</option>
            </>
        )}
        ...
        {["listlocation", "graph", "statistics"].includes(StateOnPage.status) && (
            <>
