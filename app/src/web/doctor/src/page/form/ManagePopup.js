...
// helper: สร้าง URL สำหรับ weather endpoint พร้อม st/et/r และกัน cache
const buildWeatherEndpoint = (houseId, deviceId) => {
    return `/api/sensor/weather-greenhouse/${houseId}/${deviceId}`;
};
const startOfDayLocalMs = (input) => {
...
const ManagePopup = ({ setPopup, RefPop, id_form, session, Fecth, RefData }) => {
    ...
    useEffect(() => {
        ...
        const onB = e => applyRange(e.detail);
        window.addEventListener("weather-export:set-range", onA);
        window.addEventListener("weather:set-range", onB);
        return () => {
            window.removeEventListener("weather-export:set-range", onA);
            window.removeEventListener("weather:set-range", onB);
        };
    }, []);
    ...
    useEffect(() => {
        if (!data) return;
        console.log("[POPUP] <WeatherManagement/> props", {
            endpointData: `/api/sensor/weather-greenhouse/${data?.id_farm_house}/${data?.device_id}`,
            query: weatherQuery
        });
    }, [data, weatherQuery]);
    ...
    const FetchContent = async (type_form) => {
        ...
        try {
            ...
            try {
                ...
                console.warn("โหลด sensor ไม่ได้", e);
            }

            // รวมข้อมูล และ “การันตี” ว่ามีฟิลด์ device_id อยู่ใน data เสมอ
                ...
                return (
                    <section key={key} className="profile-farmer">
                        <div className="img" style={{
                            ...
                            <img src={val.img || user_not_found}></img>
                        </div>
    ...
    return (
        <>
                ...
                <div className="content-detail">
                    {LoadContent ?
                        ...
                        <div className="content-body">
                            {
                                new Set([0, 1, 2, 4, 5]).has(TypePage) &&
                            ...
                            <div className="head-content">
                                <div className="text">{TypePage === 0 ? "ข้อมูลการปลูกพืช" : TypePage === 1 ? "ข้อมูลการใช้ปัจจัยการผลิต" : TypePage === 2 ? "ข้อมูลการใช้สารเคมี" : TypePage === 3 ? "ข้อมูลเกษตรกร" : TypePage === 4 ? "บันทึกเจ้าหน้าที่" : TypePage === 5 ? "บันทึกค่า EC/Ph" : ""}</div>
                                {TypePage === 0 ?
                                    (
                                        <a className="bt-edit" onClick={() => GetDetailEdit(id_form, "plant")}>
                                            {CountEdit ? <div className="count-edit"></div> : <></>}
                                            <svg className="icon-menu" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#2b5e55"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M1 1V15H15V1H1ZM13 9V3H3V9H6C6 10.1046 6.89543 11 8 11C9.10457 11 10 10.1046 10 9H13Z" fill="#358072"></path> </g></svg>
                                        </a>
                                    ) : <></>
                                }
                            </div>
            ...
            <div
                style={{
                    position: "fixed",
                    left: "-10000px",   // เอาออกนอกจอ
                    top: 0,
                    width: "900px",
                    height: "260px",
                    opacity: 0,         // ให้มองไม่เห็น แต่ยัง "มองว่าแสดงผล"
                    pointerEvents: "none",
                    zIndex: -1
                }}
            >
                {/* <- ตัวนี้คือ element ที่ export จะจับภาพ */}
                <div id="weather-chart-export" style={{ width: "900px", height: "260px" }}>
                    {data?.device_id && range?.st && range?.et && (
                        <WeatherManagement
                            endpointData={`/api/sensor/weather-greenhouse/${data.id_farm_house}/${data.device_id}`}
                            query={weatherQuery}
                            startTime={range.st}   // ms
                            endTime={range.et}     // ms
