...
import { io } from '../main';
import { getColumnData } from '../services/dashboard.service';
import { createMqttClientForDevice, disconnectMqttClientForDevice } from '../ttn-listener/mqtt.listener';
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const columns = await getColumnData();
        io.emit("init:columns", columns);

        await createMqttClientForDevice(created.device_id);
...
router.delete('/:id', async (req, res) => {
    ...
    disconnectMqttClientForDevice(id);
    res.sendStatus(204);
});
